...
## v12g 변경사항 — GitHub 이전 + Egress 버그 수정 + 관리자 예약자명 표시 (최신, 2026-09-05)

### 1. GitHub Pages로 호스팅 이전
- GenSpark → GitHub 저장소로 코드 이관, GitHub Pages로 배포

### 2. admin.html 30초 폴링 egress 버그 수정
- `loadFacilityStatus()`, `loadAdminReserves()`에서 `facility_reservations`/`waitlist`를 **필터 없이 전체 다운로드**하던 부분을
  다른 함수들과 동일하게 `date: eq.오늘` 서버사이드 필터로 수정 (30초마다 반복 호출되던 부분이라 영향이 컸음)

### 3. 댄스연습실/멀티룸 예약자명 표시
- 키오스크에서 관리자가 시간대를 선택해 예약할 때 예약자명/용도를 입력받지 않아 admin.html 시간표에 항상 "관리자"로만 표시되던 문제 수정
- `adminReserveWithNote()` 추가 (kiosk.js) — 시간대 선택 직후 SweetAlert로 예약자명/용도 입력 → `note`로 저장 → 시간표에 즉시 표시
- 이미 "관리자"로만 표시된 기존 예약은 admin.html 시간표에서 ✏️(편집) 버튼으로 예약자명을 채워 넣을 수 있음

### 4. 캐시 버전 범프
- 전체 5개 HTML `?v=20260905a` 적용

## v12f 변경사항 — 연간 실적 + 대기 예상시간 + 토요일 노래방 20분 (2026-09-04)
